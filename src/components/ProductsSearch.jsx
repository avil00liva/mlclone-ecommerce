...
const ProductsSearch = () => {
    ...
    const productParam = params.name
    const [productosSearch, setProductosSearch] = useState(mockProducts)
    const pSearched = productosSearch.filter((productoS)=> productoS?.name.toLowerCase().includes(`${productParam}`))
  ...
  return (
    <>
        <div className='w-full min-h-screen bg-gray-200 text-black p-4 px-4 tablet:px-8 laptop:px-16'>
            ...
            <div className='w-full min-h-screen bg-inherit flex justify-start items-start rounded-lg overflow-hidden'>
                <aside className='w-[30%] min-h-screen bg-inherit hidden laptop:block'>
                    <h2 className='text-2xl font-semibold uppercase'>{productParam}</h2>
                    <span className='text-sm tablet:text-sm text-gray-500'>{pSearched.length} resultados</span>
                ...
                <section className='w-full min-h-screen bg-white shadow-sm'>
                  {pSearched && pSearched.map((item, index)=>{
                    return (
                      <div className='w-full min-h-[200px] laptop:max-h-[200px] bg-inherit border-b border-b-gray-400 px-6 py-4 flex flex-col laptop:flex-row items-start justify-start relative'>
                        ...
                        <div className='w-full h-full bg-inherit flex flex-col justify-start'>
                          ...
                          <div className='w-full min-h-[20px] flex justify-start items-center'>
                            {item.free && <span className='text-green-500 text-sm'>Envío gratis</span>}
