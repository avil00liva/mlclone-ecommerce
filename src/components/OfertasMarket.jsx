...
const OfertasMarket = () => {
    ...
    const [ofertas, setOfertas] = useState(mockProducts)
    const relacionadas = [
      {
        id:1,
    ...
    const ofertasFilter = ofertas.filter((oferta)=>oferta.oferta === true)
        ...
        <div className='w-full min-h-screen bg-gray-100 text-black p-4 px-4 tablet:px-8 laptop:px-16'>
            ...
            <div className='w-full min-h-screen bg-inherit flex justify-start items-start rounded-lg overflow-hidden'>
                <aside className='w-[30%] min-h-screen bg-inherit hidden laptop:block'>
                  <h2 className='text-2xl font-semibold uppercase'>Ofertas</h2>
                  <span className='text-sm tablet:text-sm text-gray-500'>{ofertasFilter.length} resultados</span>
                ...
                <section className='w-full min-h-screen bg-white shadow-sm'>
                  {ofertasFilter && ofertasFilter.map((item, index)=>{
                    return (
